...
- **Fault-Tolerant API Rotation & Fallback:** Features automatic Google API key rotation (supporting 1 to 4 keys) with exponential backoff retries, and fails over to Groq structured LLMs automatically if Gemini is exhausted.
- **Custom OKLCH Theme Engine:** Supports visual personalization with dynamic theme switching (Default, Sunset, Ocean) mapped using CSS custom OKLCH color palettes.
...
- **Event-Loop Performance Optimizations:** Offloads heavy, synchronous vector database (Pinecone) queries to background worker threads and parallelizes database fetches (Supabase) via async/await pipelines to resolve N+1 latency bottlenecks.
- **Robust Test Coverage:** Backend is verified using a comprehensive pytest suite covering the API router, key rotation mechanism, and core helper utilities.
...
3. **API Key Rotator & Fallback:** Cycles Google API keys on failure with exponential backoff, failing over to Groq Llama 3.3 structured LLMs for maximum resilience.
...
![GitHub Profile Integration](static/9%20github%20overview.png)

### 10. Evidence-Bound Repository Analysis

![Evidence-Bound Repository Analysis](static/10%20github%20repo%20analysis.png)
